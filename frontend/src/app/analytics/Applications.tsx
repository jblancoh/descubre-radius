...
const Applications: FC = () => {
  ...
  return (
    ...
    >
      <h1
        className={`text-3xl font-bold mb-16 ${applicableYStyle}`}
        style={customTransitionStyles}
      >
        Aplicativos que hemos realizado
      </h1>
      <div className="container lg:px-32 flex flex-row justify-evenly lg:justify-between flex-wrap">
        {ApplicationList.map((app, index) => {
          const extraMilliseconds = 1 * index
          const customListTransitionStyles = { transition: `opacity ${0.5 + extraMilliseconds}s, transform ${0.5 + extraMilliseconds}s` }
          return (
            <Card
              key={index}
              className={`w-60 flex flex-col justify-start bg-analytics-500 border-2 border-b-8 border-b-analytics-400 rounded-none shadow-none h-80 my-5 ${applicableYStyle}`}
